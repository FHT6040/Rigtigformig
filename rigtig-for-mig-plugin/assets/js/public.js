...
(function($) {
    ...
    $(document).on('change', '#user_avatar_upload', function(e) {
        ...
        if (file.size > 2 * 1024 * 1024) {
            showNotification('Billedet må maksimalt være 2 MB', 'error');
            return;
        }
        
        // Check file type
        if (!file.type.match('image.*')) {
            showNotification('Du skal uploade et billede', 'error');
            return;
        }
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    showNotification(response.data.message, 'success');
                    $('#user-avatar-preview').attr('src', response.data.avatar_url);
                } else {
                    showNotification(response.data.message, 'error');
                }
            },
            error: function() {
                showNotification('Der opstod en fejl ved upload', 'error');
            }
        });
    });
    
    // User password change form
    $(document).on('submit', '#rfm-user-password-form', function(e) {
        e.preventDefault();
        ...
        if (!current_password || !new_password || !confirm_password) {
            showNotification('Udfyld alle felter', 'error');
            return;
        }
        
        if (new_password !== confirm_password) {
            showNotification('De nye adgangskoder matcher ikke', 'error');
            return;
        }
        
        if (new_password.length < 8) {
            showNotification('Ny adgangskode skal være mindst 8 tegn', 'error');
            return;
        }
    ...
    $(document).on('click', '#rfm-delete-account-btn', function(e) {
        e.preventDefault();
        
        // Show confirmation modal
        $('#rfm-delete-account-modal').fadeIn();
    });
    
    // Cancel delete
    $(document).on('click', '.rfm-modal-cancel, .rfm-modal-close', function() {
        $('.rfm-modal').fadeOut();
    });
    
    // Confirm delete account
    $(document).on('click', '#rfm-confirm-delete-account', function(e) {
        e.preventDefault();
        
        var password = $('#delete_account_password').val();
        
        if (!password) {
            showNotification('Indtast din adgangskode for at fortsætte', 'error');
            return;
        }
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    showNotification(response.data.message, 'error');
                    $button.prop('disabled', false).text('Slet min konto');
                }
            },
            error: function() {
                showNotification('Der opstod en fejl', 'error');
    ...
    // Download user data
    $(document).on('click', '#rfm-download-user-data', function(e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    showNotification('Dine data er downloadet', 'success');
                } else {
                    showNotification(response.data.message, 'error');
                }
                $button.prop('disabled', false).text(originalText);
